            ...
            try:
                if self._loop and self._loop.is_running():
                    self._loop.call_soon_threadsafe(
                        lambda: asyncio.create_task(manager.broadcast(verdict.to_dict()))
                    )
...
app = FastAPI(title="NeuroShield Inference Service", version="0.8.1", lifespan=lifespan)
...
def get_api_model_version() -> dict[str, Any]:
    version_data = runtime.engine.current_model_version
    return {
        "version": version_data.get("version", "unknown"),
        "validationF1": version_data.get("f1_score", 0.0),
        "lastRetrainedAt": version_data.get("timestamp"),
...
def get_api_alerts() -> list[dict[str, Any]]:
    alerts = runtime.latest_alerts(50)
    # Map to frontend expected format if needed, but currently keeping it direct
    return alerts
...
async def websocket_alerts(websocket: WebSocket):
    ...
    try:
        ...
        backlog = runtime.latest_alerts(10)
        if backlog:
